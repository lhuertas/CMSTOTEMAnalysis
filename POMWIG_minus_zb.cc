...
void POMWIG_minus_zb(string const& outputFileName = "/storage1/lhuertas/Analysis/CMSTotem_RP/CMSTotem/Workspace_new/root_files/pomwig_minus_zb.root", const Int_t nevt_max = -1){
...
//  Float_t tbins[12] = { 0.03, 0.06, 0.09, 0.12, 0.16, 0.20, 0.25, 0.30, 0.40, 0.50, 0.65, 1.};
  ...
  RooUnfoldResponse response_xminus (15, -4, 0,"unfolded_x_minus","unfolded_x_minus");
  histosTH2F["response_xminus_jjp"] = (TH2F*) response_xminus.Hresponse();
  RooUnfoldResponse response_t (histosTH1F["t_rec_proton_signal_right_kint_kinxi_cut"],histosTH1F["t_gen_proton_signal_right_kint_kinxi_cut"],"unfolded_t","unfolded_t");
  histosTH2F["response_t_right"] = (TH2F*) response_t.Hresponse();
  RooUnfoldResponse response_t_bin (50, 0, 1,"unfolded_t_bin","unfolded_t_bin");
  histosTH2F["response_t_right_bin"] = (TH2F*) response_t_bin.Hresponse();
...
*/
      ...
      double event_weight_rec = event_weight*reweigth_beta*eff_trigger*eff_proton;
      ...
      if (jet_gen && proton_minus_kinec_accept_t_gen && xi_proton_minus<=0.1 ){
          ...
          if ( !fabs(t_proton_minus_smear_rec) && fabs(t_proton_minus) ) response_t.Miss (fabs(t_proton_minus), 1.);
      }
      ...
      if (jet_rec && select_Vertex && proton_minus_kinec_accept_t_rec && xi_proton_minus_rec<=0.1)++nevents_jetrec_protkintkinxi;
      if (jet_rec && select_Vertex && proton_minus_kinec_accept_t_rec && xi_proton_minus_rec<=0.1 && rp_right)++nevents_jetrec_protkintkinxi_rp;
      if (jet_rec && select_Vertex && proton_minus_kinec_accept_t_rec && xi_proton_minus_rec<=0.1 && rp_right && xi_cms_totem_cut_minus)++nevents_jetrec_protkintkinxi_rp_cut;
      ...
      if (rp_right && jet_rec && select_Vertex){
          ...
          if (proton_minus_kinec_accept_t_rec && xi_proton_minus_rec<=0.1 && xi_cms_totem_cut_minus){
              ...
              histosTH1F["leadingJet_eta_rec_signal_kint_kinxi_cut"]->Fill( Jet1_eta_rec, event_weight_rec  );
              histosTH1F["secondJet_eta_rec_signal_kint_kinxi_cut"]->Fill( Jet2_eta_rec, event_weight_rec  );
          ...
          if ( fabs(t_proton_minus_smear_rec) && !fabs(t_proton_minus) ) response_t.Fake (fabs(t_proton_minus_smear_rec), 1.);
          ...
          if ( fabs(t_proton_minus_smear_rec) && fabs(t_proton_minus) ) response_t.Fill ( fabs(t_proton_minus_smear_rec), fabs(t_proton_minus), 1.);
              }
          }

          //response matrix
          if ( proton_minus_beta_rec && proton_minus_beta_gen ) response_beta.Fill (proton_minus_beta_rec, proton_minus_beta_gen, 1.);
          if ( !proton_minus_beta_rec && proton_minus_beta_gen ) response_beta.Miss (proton_minus_beta_gen, 1.);
          if ( proton_minus_beta_rec && !proton_minus_beta_gen ) response_beta.Fake (proton_minus_beta_rec, 1.);
          if ( x_minus_rec && x_minus_gen ) response_xminus.Fill (log10(x_minus_rec), log10(x_minus_gen), 1.);
          if ( !x_minus_rec && x_minus_gen ) response_xminus.Miss (log10(x_minus_gen), 1.);
          if ( x_minus_rec && !x_minus_gen ) response_xminus.Fake (log10(x_minus_rec), 1.);
          if ( fabs(t_proton_minus_smear_rec) && fabs(t_proton_minus) ) response_t_bin.Fill ( fabs(t_proton_minus_smear_rec), fabs(t_proton_minus), 1.);
          if ( !fabs(t_proton_minus_rec) && fabs(t_proton_minus) ) response_t_bin.Miss (fabs(t_proton_minus), 1.);
          if ( fabs(t_proton_minus_smear_rec) && !fabs(t_proton_minus) ) response_t_bin.Fake (fabs(t_proton_minus_smear_rec), 1.);
      }
    }//end loop for events
